...
<head>
	...
	<meta name="author" content="http://www.medialoot.com/" />
	<!--[if lt IE 9]>
...
<body>
	...
	<script type="text/template" id="employee_row">
		<tr> 
			...
			<td><% $.each(emp_comp, function(i, comp) {
				if (comp.employee_id == data.employee_id) print (comp.salary);
			}); %></td> 
			<td><% $.each(emp_types, function(i, etype) {
				if (etype.type_id == data.emp_type_id) print (etype.type_name);
			}); %></td> 
	...
	var form_val = $("#user_form").validate({
		...
		messages: {
			...
			emp_level: {
				required: "You must specifiy an employee level"
			}
		}
	});
	...
	compensation_url = 'compensation';
	types_url = 'employee_types';
	...
	$(document).ready(function() { 
		...
		$("#btn-usr-save").click(function() {
			...
			if ($("#user_form").valid()) {
				...
				new_e.emp_type_id =  parseInt($("#emp_type_id").val());
                new_e.type = btn_mode;
				var error = false;
				$.ajax({
					type: "POST",
	...
	function loadCompensation() {
		$.getJSON(compensation_url+"/calccompensation")
		.done(function(data) { emp_comp = data.result.items; showLocations(); })
		.fail(function() { showMessage("error","Could not load location list."); });
	}
	function loadEmployeeTypes() {
		$.getJSON(types_url)
		.done(function(data) { emp_types = data.result.items; showTypes(); })
		.fail(function() { showMessage("error","Could not load location list."); });
	}
	...
	function showTypes() {
		$.each(emp_types, function(key, value) {   
			$('#emp_type_id')
				.append($("<option></option>")
	...
	function loadEmployees()
    {
		...
		$.getJSON(users_url)
         ...
         .fail(function() { showMessage("error","Could not load messages."); });
		/*
		var loc_def = $.getJSON(compensation_url),
		comp_def = $.getJSON(compensation_url),
		emp_type_def = $.getJSON(types_url),
		emp_def = $.getJSON(users_url);
		$.when(loc_def, comp_def, emp_type_def, emp_def)
		.done(function(loc_response, comp_reponse, emp_type_response, emp_reponse) { 
			loc_list = loc_response.result.items; 
			showLocations();
			emp_comp = comp_reponse.result.items; 
			emp_types = emp_type_response.result.items;
			showTypes();
			employeeData = emp_reponse.result.items; 
			displayEmployeeData();
		.fail(function() { showMessage("error","Could not load employee data."); });
		*/
	}
